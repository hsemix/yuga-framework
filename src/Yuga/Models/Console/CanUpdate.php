    ...
    protected function makeUpdateForm(Model $model)
    {
        ...
        foreach ($model->scaffold as $fieldName => $type) {
            ...
            if ($fieldType != 'password') {
                $input = '<input name="'. $fieldName .'" type="'. $fieldType .'" value="{{ $' . $property . '->' . $fieldName . ' }}" class="form-control" />';
            }
            if ($fieldType == 'textarea') {
                $input = '<textarea name="'. $fieldName .'" cols="10" rows="4" class="form-control">{{ $' . $property . '->' . $fieldName . ' }}</textarea>';
            }

            $label = \ucfirst($fieldName);
            $inputs .= '<div class="form-group">
                    <label class="control-label">'. ucfirst($fieldName) .'</label>
                    ' . $input . '
        ...
        $creator = str_replace(
            ...
            [$name, $inputs, 'Edit', Inflect::pluralize($property)], 
